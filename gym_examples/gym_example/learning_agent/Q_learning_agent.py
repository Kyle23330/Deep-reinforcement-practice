...
if __name__ == '__main__':
    ...
    for episodes in range(5000):
        ...
        while not terminated:
            action = Q_learning_agent.get_action(state = cur_state)
            observation, reward_change, terminated, truncated, info = env.step(action)

            next_state = observation["agent"]
            action_next = Q_learning_agent.get_action(next_state)
            Q_learning_agent.update(cur_state,action,reward_change,next_state,terminated)
    ...
    plt.plot(range(5000),all_rewards)
    plt.show()